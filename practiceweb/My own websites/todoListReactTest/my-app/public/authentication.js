...
exports.loadAuthentication = (passport) => {
    console.log('authentication has been loaded');
//local strategy
...
passport.use(new LocalStrategy({usernameField: 'email'}, async (email, password, done) => {
    try{
       ...
       // console.log(`bcrypt compare returns ${await !bcrypt.compare(password, user.password)}`);
        ...
        console.log(`the password is ${bcrypt.compare(password, user.password)}`);
        console.log(`the user database password is ${user.password}`);
